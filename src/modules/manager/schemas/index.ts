...
export const addTeamMemberSchema = Joi.object({
  ...
  role: Joi.string().valid('employee', 'senior_employee', 'lead', 'specialist').required(),
  position: Joi.string().min(1).optional().allow(''),
  salary: Joi.number().min(0).optional().allow(0),
  startDate: Joi.date().iso().optional(),
  address: Joi.string().min(10).optional().allow(''),
  emergencyContact: Joi.string().min(2).optional().allow(''),
  emergencyPhone: Joi.string().min(10).optional().allow(''),
...
export const approvalStatsSchema = Joi.object({
  startDate: Joi.date().iso().optional(),
  endDate: Joi.date().iso().optional(),
  department: Joi.string().optional().allow(''),
...
// Leave Policy Schemas (Read-only access for managers)
